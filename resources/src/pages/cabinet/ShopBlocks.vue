<template>
    <div class="cabinet-block">
        <h4 class="cabinet-block__name">Магазин блоков</h4>

        <div class="row g-20px">
            <div class="col-12 col-xl-6 col-xxl-4" v-for="block in blocks">
                <div class="shop-block">
                    <h5 class="shop-block__text">{{ block.name }}</h5>
                    <div class="shop-block__image">
                        <img :src="block.image" alt="" />
                    </div>
                    <div class="shop-block__footer">
                        <p class="shop-block__text">{{ block.price }}₽</p>
                        <button
                            class="button button__primary px-big mini"
                            @click="openModalBuy(block)"
                        >
                            Купить
                        </button>
                    </div>
    ...
    >
        ...
        <h1 class="custom-modal__title">Покупка {{ dataBlock.name }}</h1>

        <div class="d-flex flex-column gap-30px">
            <div class="row g-28px align-items-center">
                <div class="col-auto">
                    <div class="block__image">
                        <img :src="dataBlock.image" alt="" />
                    </div>
                </div>
                <div class="col">
                    <div class="cabinet-block">
                        <div class="table-dotted">
                            ...
                            <div
                                ...
                                v-if="dataBlock.item_id"
                            >
                                <p class="table-dotted__name">ID предмета:</p>
                                <p
                                    class="table-dotted__value icon-after"
                                    @click="copyText(dataBlock.item_id)"
                                >
                                    {{ dataBlock.item_id }}
                                </p>
                            </div>
                            ...
                            >
                                ...
                                <p
                                    class="table-dotted__value icon-after"
                                    @click="copyText(dataBlock.item_type)"
                                >
                                    {{ dataBlock.item_type }}
                                </p>
                            </div>
...
function openModalBuy(blockId) {
    dataBlock.value = blockId;
    modalBuy.value = true;
    count.value = 1;
}

function buy(blockId) {
    api.post("/shop/buy/block", {
        id: blockId,
        count: count.value,
